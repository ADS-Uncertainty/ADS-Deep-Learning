...
class ngsimDataset(Dataset):
    def __init__(self, data_traj, data_tracks, t_h=30, t_f=50, d_s=2, enc_size = 64, grid_size = (13,3)):

        self.D = torch.load(data_traj)
        self.T = np.load(data_tracks, allow_pickle=True)
        self.t_h = t_h
        self.t_f = t_f
        self.d_s = d_s
    ...
    def __getitem__(self, idx):
        dsId = self.D[idx, 0].to(torch.int)
        vehId = self.D[idx, 1].to(torch.int)
        t = self.D[idx, 2]
        grid = self.D[idx,8:]
        neighbors = []
        ...
        for i in grid:
            neighbors.append(self.get_nbrs_History(i.to(torch.int), t, vehId, dsId))
    ...
    def getHistory(self,vehId,t,refVehId,dsId):
        ...
        if vehId == 0:
            ...
            if vehTrack.size == 0 or np.argwhere(vehTrack[:, 0] == t).size == 0:
                ...
                stpt = np.maximum(0, np.argwhere(vehTrack[:, 0] == t).item() - self.t_h)   # t_h=30
                enpt = np.argwhere(vehTrack[:, 0] == t).item() + 1
                hist = vehTrack[stpt:enpt:self.d_s, 1:3]-refPos    # d_s=2
    ...
    def get_nbrs_History(self,vehId,t,refVehId,dsId):
        if vehId == 0:
            return np.empty([0,2])
        else:
            if self.T.shape[1] <= vehId-1:
    ...
    def getFuture(self, vehId, t,dsId):
        ...
        enpt = np.minimum(len(vehTrack), np.argwhere(vehTrack[:, 0] == t).item() + self.t_f + 1)   # t_f=50
    ...
    def collate_fn(self, samples):
        ...
        for _,_,nbrs,_,_ in samples:
            nbr_batch_size = nbr_batch_size + sum([len(nbrs[i])!=0 for i in range(len(nbrs))])
...
def maskedNLL(y_pred, y_gt, mask):
    ...
    out = 0.5*torch.pow(ohr, 2)*(torch.pow(sigX, 2)*torch.pow(x-muX, 2) + torch.pow(sigY, 2)*torch.pow(y-muY, 2) - 2*rho*(x-muX)*(y-muY)*torch.pow(sigX, 1)*torch.pow(sigY, 1)) - torch.log(sigX*sigY*ohr) + 1.8379
...
def maskedMSETest(y_pred, y_gt, mask):
    acc = torch.zeros_like(mask)     # torch.Size([25, 128, 2])
    muX = y_pred[:, :, 0]
    muY = y_pred[:, :, 1]
    x = y_gt[:, :, 0]
    y = y_gt[:, :, 1]
...
def maskedADETest(y_pred, y_gt, mask):
    acc = torch.zeros_like(mask)     # torch.Size([25, 128, 2])
    muX = y_pred[:, :, 0]
    muY = y_pred[:, :, 1]
    x = y_gt[:, :, 0]
...
def maskedsingleTest(y_pred, y_gt, mask):
    # ===========================================================
    # single x,y
    acc = torch.zeros_like(mask)     # torch.Size([25, 128, 2])
    muX = y_pred[:, :, 0]
    ...
    torch.pow(torch.pow(x - muX, 2), 0.5)
    acc[:, :, 0] = torch.pow(torch.pow(x - muX, 2), 0.5)
    acc[:, :, 1] = torch.pow(torch.pow(y - muY, 2), 0.5)
    acc = acc * mask
    lossVal1 = torch.sum(acc[:,:,0],dim=1)
    lossVal2 = torch.sum(acc[:, :, 1], dim=1)
    counts = torch.sum(mask[:,:,0],dim=1)
    return lossVal1, lossVal2, counts