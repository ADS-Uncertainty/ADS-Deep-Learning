...
from LSTM_CSP_util import outputActivation
...
class highwayNet(nn.Module):
    def __init__(self,args):
        ...
        # Dropout layer: Uncertainty Quantification Dropout Ratio
    ...
    def forward(self, hist, nbrs, masks, lat_enc, lon_enc):
        ...
        hist_enc = self.dropout(hist_enc)  # Mc Dropout
        hist_enc = self.leaky_relu(self.dyn_emb(hist_enc.view(hist_enc.shape[1], hist_enc.shape[2])))
        # hist_enc = F.dropout(hist_enc, p=self.dropout_probability, training=self.training)

        # nbrs
        _, (nbrs_enc, _) = self.enc_lstm(self.leaky_relu(self.ip_emb(nbrs)))
        nbrs_enc = self.dropout(nbrs_enc)  # Mc Dropout
